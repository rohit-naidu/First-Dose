      ...
      <div className="flex gap-1 mb-6 p-1 rounded-xl w-fit" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(240,244,248,0.08)' }}>
        {[{ id: 'overview', label: 'Overview' }, { id: 'intake-report', label: 'Intake Report' }].map(tab => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id)}
            ...
            style={{
              background: activeTab === tab.id ? 'rgba(127,181,201,0.12)' : 'transparent',
              color: activeTab === tab.id ? '#7fb5c9' : 'rgba(240,244,248,0.45)',
              border: activeTab === tab.id ? '1px solid rgba(127,181,201,0.25)' : '1px solid transparent',
            }}>
            {tab.label}
          </button>
        ))}
      ...
      {activeTab === 'overview' && (
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div className="rounded-lg px-4 py-3"
            ...
            <p className="text-xs" style={{ color: 'rgba(127,181,201,0.8)' }}>
              Patient is progressing on standard protocol. No adverse events reported. Last check-in: {patient.lastCheckin}.
            </p>
          </div>
        </motion.div>
      )}
